...

{
    ...
    public function index_get()
    {
        ...
        if ($user_id)
        {
            return $this->success_response(
                'Data order user berhasil diambil',
                $this->Order_model->get_user_orders($user_id)
            );
        }
    ...
    public function cancel_put($id)
    {
        if (!$this->Order_model->get_order($id))
        {
            return $this->error_response('Order tidak ditemukan', null, 404);
        }

        $cancelled = $this->Order_model->cancel_order($id);

        if (!$cancelled)
        {
            return $this->error_response('Gagal membatalkan order. Order mungkin sudah dibatalkan atau status tidak memungkinkan.', null, 400);
        }
